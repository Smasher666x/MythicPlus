...
function MythicHandlers.RequestVaultStatus()
    AIO.Handle("AIO_Mythic", "RequestVaultStatus")
end

local function CheckMythicAfterReload()
    C_Timer.After(3.0, function()
        MythicHandlers.RequestVaultStatus()
    end)
end
...
function MythicHandlers.RestoreMythicTimerGUI(_, mapId, tier, duration, bossNames, potentialGain, enemiesRequired, elapsedTime, killedBosses, currentDeaths, enemyProgress)
    potentialGain = tonumber(potentialGain) or 0
    enemiesRequired = tonumber(enemiesRequired) or 50
    elapsedTime = tonumber(elapsedTime) or 0
    currentDeaths = tonumber(currentDeaths) or 0
    enemyProgress = tonumber(enemyProgress) or 0
    
    if type(bossNames) ~= "table" then bossNames = {} end
    if type(killedBosses) ~= "table" then killedBosses = {} end
    ...
    local function updateProgress(killedBossCount, totalBosses)
        if totalBosses == 0 then return end
        local progress = math.min(killedBossCount / totalBosses, 1.0)
        local maxWidth = 256
        local currentWidth = maxWidth * progress
        if progress > 0 then
            progressBar:Show()
    ...
    timerText:SetText(fmt("%02d:%02d", floor(timeLeft/60), floor(timeLeft%60)))

    local deaths, penalty, bonus = currentDeaths, currentDeaths * 10, 0
    local scoreLabel = timerFrame:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
    scoreLabel:SetPoint("BOTTOM", goldenFrame, "BOTTOM", 60, 50)
    ...
    local bossLabels = {}
    local killedBossCount = 0
    
    for i, name in ipairs(bossNames) do
        local lbl = timerFrame:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        lbl:SetPoint("TOPLEFT", goldenFrame, "BOTTOMLEFT", 20, 10 - (i-1)*16)
        local isKilled = false
        for _, killedIndex in ipairs(killedBosses) do
            if killedIndex == i then
                isKilled = true
                killedBossCount = killedBossCount + 1
                break
            end
        end
        if isKilled then
            lbl:SetText(fmt("1/1 %s", name))
            lbl:SetTextColor(0.15, 0.76, 0.15)
        else
            lbl:SetText(fmt("0/1 %s", name))
            lbl:SetTextColor(1, 0.82, 0)
        end
        lbl.bossName = name
        bossLabels[i] = lbl
    ...
    if enemiesRequired > 0 then
        ...
        local enemyCompleted = enemyProgress >= 100
        local completedText = enemyCompleted and "1" or "0"
        enemyLabel:SetText(fmt("%s/1 Enemy Forces", completedText))
        
        if enemyCompleted then
            enemyLabel:SetTextColor(0.15, 0.76, 0.15)
        else
            enemyLabel:SetTextColor(1, 0.82, 0)
        end
        
        enemyProgressFrame = CreateFrame("StatusBar", nil, timerFrame)
        enemyProgressFrame:SetSize(200, 8)
        enemyProgressFrame:SetPoint("TOPLEFT", enemyLabel, "BOTTOMLEFT", 0, -5)
        enemyProgressFrame:SetStatusBarTexture("Interface\\TargetingFrame\\UI-StatusBar")
        enemyProgressFrame:SetStatusBarColor(0.2, 0.8, 0.2)
        enemyProgressFrame:SetMinMaxValues(0, 100)
        enemyProgressFrame:SetValue(enemyProgress)
        ...
        enemyPercentText = enemyProgressFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
        enemyPercentText:SetPoint("CENTER", enemyProgressFrame, "CENTER")
        enemyPercentText:SetText(fmt("%.0f%%", enemyProgress))
    end
    ...
    updateProgress(killedBossCount, #bossNames)
